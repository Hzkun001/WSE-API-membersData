...
export function getById(req, res) {
  ...
  if (!item) return res.status(404).json(fail("Member tidak ditemukan"));
...
export function replace(req, res) {
  ...
  if (idx === -1) return res.status(404).json(fail("Member tidak ditemukan"));
...
export function remove(req, res) {
  ...
  list.splice(idx, 1);
  saveData(list);
  return res.status(200).json(ok(deleted, "Deleted"));
}
